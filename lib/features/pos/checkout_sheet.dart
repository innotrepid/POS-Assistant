...
class _CheckoutSheetState extends State<CheckoutSheet> {
  /// null = single-method mode using [_paymentType]
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final isCreditSale = _isCredit ||
          (_splitMode && _splitRemaining > 0.001 && _customer != null);
      ...
      if (_splitMode) {
        ...
        if (_splitRemaining > 0.001) {
          // Remainder must be explicit credit with customer
          if (_customer == null) {
            throw ArgumentError(
      ...
      final useCredit = _isCredit ||
          (_splitMode && _splitRemaining > 0.001);
  ...
  Widget _buildSplitSection() {
    ...
    return Column(
      ...
      children: [
        ...
        if (over)
          Text(
            'Over by ${Money.format(_splitPaid - _total)} — reduce a leg',
            style: TextStyle(color: Theme.of(context).colorScheme.error),
          )
          ...
          _customerTile(
            subtitle:
                'Select customer to put remaining ${Money.format(remaining)} on credit',
          ),
        ] else
          const Text(
            'Fully covered',
  ...
  Widget _customerTile({String? subtitle}) {
    return ListTile(
      ...
      subtitle: Text(
        subtitle ??
            (_customer?.phone ?? 'Required when balance remains'),
      ),
