  ...
  Future<SaleCreateResult> createSale({
    ...
    await db.transaction((txn) async {
      for (final item in items) {
        ...
        final need = item.stockQuantity;
        if (currentStock < need && !allowNegative) {
          throw StateError(
            'Insufficient stock for ${item.productName}. '
            'Available: ${_formatNumber(currentStock)}, '
            'requested: ${_formatNumber(need)} (base units). '
      ...
      for (final item in items) {
        ...
        final stockQty = item.stockQuantity;
        final displayName = item.isQuickSale
        ...
        if (!item.isQuickSale) {
          await txn.insert('stock_movements', {
            ...
            'unit_cost': unitCost,
            'reference_id': saleId,
            'reason': item.unitName != null
                ? 'Sale completed (${item.quantity} ${item.unitName})'
                : 'Sale completed',
            'created_at': now,
          });
        }
  ...
  Future<SaleVoidResult> voidSale({
    ...
    await db.transaction((txn) async {
      for (final item in items) {
        ...
        final qty = (baseQty != null && baseQty > 0) ? baseQty : soldQty;
        if (qty <= 0) continue;
